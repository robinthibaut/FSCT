...
def augmentations(x, y, min_sample_points):
    ...
    def random_point_removal(x, y, min_sample_points):
        ...
        num_points_to_keep = min_sample_points + int(
            np.random.uniform(0, 0.95) * (np.shape(x)[0] - min_sample_points)
        )
    ...
    if np.random.uniform(0, 1) >= 0.5 and x.shape[0] > min_sample_points:
        x, y = subsample_point_cloud(
            x, y, np.random.uniform(0.01, 0.025), min_sample_points
        )
