...
def combine_multiple_output_CSVs(point_clouds_to_process, csv_file_to_combine):
    combined_plot_summary_dataframe = pd.read_csv(
        point_clouds_to_process[0][:-4] + "_FSCT_output/" + csv_file_to_combine,
        index_col=[0],
    )
    if len(point_clouds_to_process) > 1:
        for point_cloud_filename in point_clouds_to_process[1:]:
            ...
            combined_plot_summary_dataframe = pd.concat(
                [
                    combined_plot_summary_dataframe,
                    pd.read_csv(report_filename, index_col=[0]),
                ]
            )
...
if __name__ == "__main__":
    ...
    combined_plot_summary_dataframe = combine_multiple_output_CSVs(
        point_clouds_to_process, "plot_summary.csv"
    )
    combined_plot_summary_dataframe.to_csv(
        output_directory + "/combined_plot_summaries.csv", sep=","
    )

    combined_tree_data = combine_multiple_output_CSVs(
        point_clouds_to_process, "tree_data.csv"
    )
    combined_tree_data.to_csv(output_directory + "/combined_tree_data.csv", sep=",")

    combined_taper_data = combine_multiple_output_CSVs(
        point_clouds_to_process, "taper_data.csv"
    )
